...
# ==========================================
# NUEVAS RUTAS DE ACTUALIZACIÓN Y ELIMINACIÓN
# ==========================================

@app.post("/estaciones/", status_code=201, tags=["Estaciones"], summary="Registrar una nueva estación (PROTEGIDO)")
def crear_estacion(
    datos: dict, # Cambiamos schemas.EstacionCreate por dict para que reciba el JSON de Flutter sin trabas
    db: Session = Depends(database.get_db),
    usuario: str = Depends(obtener_usuario_actual)
):
    # Extraemos los campos del diccionario enviado por Flutter
    estacion_id = datos.get("id")
    nombre = datos.get("nombre")
    ubicacion = datos.get("ubicacion")

    if not estacion_id or not nombre:
        raise HTTPException(status_code=400, detail="Faltan campos obligatorios (id o nombre)")
